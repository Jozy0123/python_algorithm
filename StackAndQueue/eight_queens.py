...
if __name__ == "__main__":
    ...
    while True:
        if len(queens_list) < 9:
            # print(queens_list)
            latest_pos = stack.pop()
            queens_list.pop()
            if len(queens_list) == 0:
                init_queen = chessBoard.get_next_pos(latest_pos)
                if validate_pos(init_queen):
                    stack.push(init_queen)
            ...
            while validate_pos(next_pos) and len(queens_list) < 9:
                if not validate_pos(next_pos):
                    break
                else:
